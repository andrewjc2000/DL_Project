...
class Data(Dataset):
    def __init__(self, dataset_name, root, train=True, sequence_length=0, mode="last",
        ...
        self.root = root #os.path.join(root, dataset_name)
        self.train = train
    ...
    def __initialize(self):
        ...
        for i, video_path in enumerate(sorted(videos)):
          if (video_path[-2:] != 'gt') and (video_path[-2:] != '.m') and (video_path[-2:] != 'm~'):
              self.videos[i] = {}
        ...
        if need_border_padding == 0 and offset == 0:
            ...
            if self.mode == 'last':
                if need_border_padding > 0 and np.abs(offset) > 0:
                    print('The video is too short or the context frame number is too large!')
                    raise NotImplementedError
                # print("Val1:",start_idx, start_idx - offset, end_idx)
                if start_idx > 0:
                  offset = -1
                idx = [x for x in range(start_idx - offset, end_idx + 1)]
                # print("Val2:",idx, offset, need_border_padding)
...
class DataWithFlows(Dataset):
    def __init__(self, dataset_name, root, train=True, sequence_length=0, mode="last",
                 all_bboxes=None, patch_size=224, normalize=False, bboxes_extractions=False):
        super(DataWithFlows, self).__init__()
        self.dataset_name = dataset_name
        self.root = root #os.path.join(root, dataset_name)
        self.train = train
        self.videos = OrderedDict()
    ...
    def __initialize(self):
        if self.train:
            self.root_frames = os.path.join(self.root, 'Train', '*')
            self.root_frames_flows = os.path.join(self.root, 'Train', 'flows/*')
            # print(self.root_frames)
            # print(self.root_frames_flows)
        else:
            self.root_frames = os.path.join(self.root, 'Test', '*')
            self.root_frames_flows = os.path.join(self.root, 'Test', 'flows/*')
            # print(self.root_frames)
            # print(self.root_frames_flows)
        ...
        videos = glob.glob(self.root_frames)
        for i, video_path in enumerate(sorted(videos)):
          if (video_path[-2:] != 'gt') and (video_path[-2:] != '.m') and (video_path[-2:] != 'm~') and (video_path[-1:] != 's'):
          # if ():
            self.videos[i] = {}
            self.videos[i]['path'] = video_path
            self.videos[i]['frames'] = glob.glob(os.path.join(video_path, '*.tif'))
    ...
    def __getitem__(self, index):
        ...
        for i in frame_idx_range:
            # [h,w,c] -> [c,h,w] BGR
            cur_img = np.transpose(cv2.imread(self.frame_addresses[i]), [2, 0, 1])
            cur_flow = np.transpose(np.load(self.frame_addresses_flows[i], allow_pickle=True), [2, 0, 1])
            img_batch.append(cur_img)
